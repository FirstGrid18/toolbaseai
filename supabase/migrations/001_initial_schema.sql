...
-- ── TOOLS ────────────────────────────────────────────────────────────────────
-- Public read only. No user INSERT / UPDATE / DELETE.
...
-- ── REVIEWS ──────────────────────────────────────────────────────────────────
-- SELECT  : approved rows only (unapproved are invisible to the public).
-- INSERT  : allowed, but `approved` MUST be false — users cannot self-approve.
--           `id` and `created_at` are excluded from the check because they are
--           generated by the DB and never supplied by the caller.
-- UPDATE  : no policy → blocked by RLS for all users.
...
create policy "Anyone can submit a review"
  ...
  with check (
    approved = false   -- callers may not set approved = true
  );

-- ── SUBMISSIONS ───────────────────────────────────────────────────────────────
-- INSERT  : allowed, but `status` MUST arrive as 'pending' — callers cannot
--           pre-approve or reject their own submission.
-- UPDATE  : no policy → blocked by RLS for all users.
-- DELETE  : no policy → blocked by RLS for all users.
...
create policy "Anyone can submit a tool"
  ...
  with check (
    status = 'pending'  -- callers may not set status to 'approved' or 'rejected'
  );
